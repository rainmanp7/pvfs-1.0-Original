/* pvfs_mkdir.c copyright (c) 1996 Clemson University, all rights reserved.
...
static char pvfs_mkdir_c[] = "$Header: /projects/cvsroot/pvfs/lib/pvfs_mkdir.c,v 1.6 2000/10/30 14:44:49 rbross Exp $";
...
int pvfs_mkdir(const char* pathname, int mode)
{
	...
	if (pvfs_checks_disabled) return mkdir(pathname, mode);
	...
	request.req.mkdir.fname = 0;
	request.req.mkdir.mode = mode;

	/* Send request to mgr */	
	if ((i = send_mreq_saddr(saddr, &request, fn, &ack)) < 0) {
		PERROR("send_mreq_dmeta");
		return(-1);
	}
	return (0);
}
