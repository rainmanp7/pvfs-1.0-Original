/* pvfs_ftruncate64.c copyright (c) 1998 Clemson University,
	all rights reserved.
...
static char pvfs_ftruncate64_c[] = "$Header: /projects/cvsroot/pvfs/lib/pvfs_ftruncate64.c,v 1.2 1999/08/13 23:26:21 rbross Exp $";
...
int pvfs_ftruncate64(int fd, int64_t length)
{
	...
	if (!pfds[fd] || pfds[fd]->fs==FS_UNIX) {
		if ((off_t) length == length) {
			return(syscall(SYS_ftruncate, fd, (off_t) length));
		}
		else {
			errno = EINVAL;
			return(-1);
